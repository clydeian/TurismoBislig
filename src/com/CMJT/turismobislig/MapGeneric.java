 ...
 * 
 */
public class MapGeneric extends FragmentActivity implements
		OnMyLocationChangeListener, LocationListener {

	private final String TAG = "MapGeneric";
	...
	private static final LatLng Babano = new LatLng(8.18839, 126.35602);
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		Log.v(TAG, "Setting up map data here");
		Bundle bundle = getIntent().getParcelableExtra("bundle");
		mapLatLng = bundle.getParcelable("mapLatLng");
		mapTitle = bundle.getString("mapTitle");
		
		Log.v(TAG, "Just a check of what we are getting here");
		Log.v(TAG, "maplat = " + mapLatLng.latitude);
		Log.v(TAG, "maplong = " + mapLatLng.longitude);
		Log.v(TAG, "maptitle = " + mapTitle);
		...
		if (status != ConnectionResult.SUCCESS) { // Google Play Services are
			...
			mMap.setOnInfoWindowClickListener(new OnInfoWindowClickListener() {
				...
				public void onInfoWindowClick(Marker marker) {
					...
					Intent intent = new Intent(MapGeneric.this, cls);
			...
			// Creating a LatLng object for the Babano location
			latLng1 = this.latLng;
			...
			rgModes.setOnCheckedChangeListener(new OnCheckedChangeListener() {
				...
				public void onCheckedChanged(RadioGroup group, int checkedId) {
					...
					LatLng dest = mapLatLng;

					// Getting URL to the Google Directions API
					String url = MapGeneric.this.getDirectionsUrl(origin, dest);
	...
	private class DownloadTask extends AsyncTask<String, Void, String> {
		...
		protected void onPreExecute() {
			// TODO Auto-generated method stub
			super.onPreExecute();
			progressDialog = new ProgressDialog(MapGeneric.this);
		...
		protected String doInBackground(String... url) {
			...
			try {
				...
				data = MapGeneric.this.downloadUrl(url[0]);
			} catch (Exception e) {
	...
	private class ParserTask extends
		...
		protected void onPostExecute(List<List<HashMap<String, String>>> result) {
			...
			if (result.size() < 1) {
				Toast.makeText(MapGeneric.this.getBaseContext(),
						"You are out of navigation bounds", Toast.LENGTH_SHORT)
						.show();
				return;
			}
			...
			if (mMode == MODE_DRIVING)
				MapGeneric.this.tvDistanceDuration.setText("Distance:"
			...
			poly = MapGeneric.this.mMap.addPolyline(lineOptions);
		}
	}
